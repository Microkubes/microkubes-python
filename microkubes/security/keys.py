...
class Key:
    ...
    :raises: :class:`KeyException` if the key file does not exist or is not a file.
    """
    def __init__(self, key_file, public=False):
        self.public = public
        self.key_file = key_file
        if not isfile(self.key_file):
            raise KeyException('File %s does not exist or is not a file.' % self.key_file)
    ...
    def load(self):
        ...
        :raises: :class:`KeyException` if an error occurs during loading.
        """
        try:
            with open(self.key_file, 'rb') as kf:
                return kf.read()
        except Exception as read_err:
            raise KeyException from read_err
    ...
    def _load_keys_from_map(self, keys):
        for key_name, key_file in keys.items():
            if not isfile(key_file):
                raise KeyException('Key file %s does not exist or it is not a file.' % key_file)
